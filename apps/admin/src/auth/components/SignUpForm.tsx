...
import { Mail, Lock, Eye, EyeOff, Shield, Github, Chrome, Compass, User, Fingerprint } from "lucide-react";

interface SignUpFormProps {
  onSignUp: (fullName: string, email: string, password: string, invitationToken: string) => Promise<void>;
  onSuccess: () => void;
  signInPath?: string;
}
...
export function SignUpForm({
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      if (!value.invitationToken.trim()) {
        setError("Operator registration requires a valid authorization token.");
      ...
      try {
        ...
        setSuccessMsg("Operator account created! Sign-in verification pending Super Admin activation.");
        setTimeout(() => {
          onSuccess();
        }, 3000);
      } catch (err: any) {
        setError(err.message || "Invitation registration failed. Token may be invalid or expired.");
      } finally {
        setIsSubmitting(false);
      }
    },
            ...
            <form.Field
              ...
              children={(field) => (
                <div className="flex flex-col gap-1">
                  ...
                  <div className="relative">
                    <Fingerprint className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/60 size-4 pointer-events-none" />
                    <Input
                      id={field.name}
                      name={field.name}
                      value={field.state.value}
