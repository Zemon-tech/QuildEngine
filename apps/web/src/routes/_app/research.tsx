...
const mockPapers: Paper[] = [
  {
    ...
    aiSummary:
      "This paper outlines the foundation of LLM scaling. It demonstrates that model performance increases predictably in relation to parameters, dataset size, and compute, establishing power-law relations that guided the development of GPT-3 and subsequent models.",
  ...
  },
  {
    ...
    likes: 512,
    abstract:
      "An inquiry into visual representation of neural activation tokens. We structure interactive node coordinates in real-time, matching concept relationships across multi-dimensional cluster projection maps.",
    keyFindings: [
    ...
    aiSummary:
      "The authors introduce a high-fidelity semantic graph visualizer that projects token activations onto geometric clusters. This model explains how dense state vectors capture conceptual relationships.",
  ...
  },
  {
    ...
    aiSummary:
      "This paper catalogs how larger model sizes enable capabilities that cannot be predicted by extrapolating performance from smaller versions, shifting execution rates from random guess to high accuracy.",
  ...
  },
  {
    ...
    likes: 390,
    abstract:
      "This paper introduces design paradigms for multiplayer scientific research workspaces. We evaluate CRDT conflict resolution models on canvas whiteboard coordinates and remote cursor sharing under variable network latency.",
    keyFindings: [
    ...
    aiSummary:
      "The authors outline concurrency handling for scientific whiteboard notebooks. They prove that custom CRDT coordinates solve collaborative overlay conflicts without backend locking.",
  ...
  },
  {
    ...
    likes: 670,
    abstract:
      "We present a framework for verifiable code synthesis using execution-guided symbolic assertions. By embedding automated correctness checks directly into the LLM decoding loop, we guarantee syntactic correctness and logical alignment with specifications.",
    keyFindings: [
    ...
    aiSummary:
      "Outlines a synthesis compiler integration that checks code correctness mid-generation. It ensures created functions compile and pass unit tests before final confirmation.",
    ...
    figure: (
      <div className="space-y-4 p-4 bg-[var(--sb-bg)]/80 rounded-xl border border-[var(--sb-border)]/30 font-sans">
        <div className="flex items-center justify-between border-b border-[var(--sb-border)]/30 pb-2">
          <span className="text-[10px] font-mono text-[var(--sb-ink-dim)] uppercase">
            Verification Pipeline Telemetry
          </span>
          <span className="text-[9px] bg-green-500/10 text-green-500 border border-green-500/20 px-2 py-0.5 rounded font-mono font-bold">
            Passed
          </span>
        </div>
        <div className="space-y-3 font-mono text-xs">
          <div className="flex items-start gap-2.5">
            <div className="size-4 rounded-full bg-indigo-500/20 border border-indigo-500 text-indigo-500 flex items-center justify-center text-[9px] font-black shrink-0 mt-0.5">
              1
            </div>
            <div className="space-y-1 font-sans">
              <div className="text-[var(--sb-ink)] font-bold text-xs">
                Generate Code Candidate
              </div>
              <div className="text-[var(--sb-ink-dim)] text-[10px]">
                Model outputs: `def solve(n): return [i*i for i in range(n)]`
              </div>
            </div>
          </div>
          <div className="flex items-start gap-2.5">
            ...
            <div className="space-y-1 font-sans">
              <div className="text-[var(--sb-ink)] font-bold text-xs">
                Inject Execution Assertions
              </div>
              <div className="text-[var(--sb-ink-dim)] text-[10px]">
                Appended unit test check: `assert solve(3) == [0, 1, 4]`
              </div>
            </div>
          </div>
          <div className="flex items-start gap-2.5">
            <div className="size-4 rounded-full bg-green-500/20 border border-green-500 text-green-500 flex items-center justify-center text-[9px] font-black shrink-0 mt-0.5">
              ✓
            </div>
            <div className="space-y-1 font-sans">
              <div className="text-[var(--sb-ink)] font-bold text-xs">
                Assertion Passed (0ms)
              </div>
              <div className="text-[var(--sb-ink-dim)] text-[10px]">
                Sandbox execution succeeded. Candidate verified and committed.
              </div>
            </div>
          </div>
        </div>
  ...
  },
  {
    ...
    aiSummary:
      "The breakthrough paper introducing ResNets. By adding shortcut skip-connections that perform identity mapping, the authors solved the vanishing gradient problem, allowing networks to grow deeper and more accurate.",
...
const mockOrganizations: Organization[] = [
  {
    ...
    logo: "OA",
    about:
      "Research and deployment company dedicated to ensuring that artificial general intelligence benefits all of humanity.",
  ...
  },
  {
    ...
    logo: "AG",
    about:
      "Advanced scientific research facility engineering collaborative notebook compilers, concept coordinates, and sandboxed symbolic execution code generators.",
  ...
  },
  {
    ...
    logo: "GR",
    about:
      "Tackling challenges that define the next generation of computing and technology, with global labs pushing algorithms boundaries.",
...
const mockCollections: Collection[] = [
  {
    ...
    description:
      "Curated papers detailing autoregressive laws, emerging benchmarks, and topological network graph activations.",
    papersCount: 3,
  ...
  },
  {
    ...
    description:
      "Multiplayer scientific whiteboard editors, sandboxed synthesis assertions, and ResNets optimization.",
    papersCount: 3,
    papers: [
      "collaborative-notebooks",
      "verifiable-code-synthesis",
      "deep-residual",
    ],
  },
];
...
const mockSearchItems: SearchItem[] = [
  ...
  },
  {
    ...
    type: "Paper",
    title: "Concept Graph",
    creator: "Elena Rostova & Liam Vance",
    domain: "Artificial Intelligence",
    difficulty: "Advanced",
    readTime: "11 min",
  },
  ...
  },
  {
    id: "consistent-hashing",
    type: "Article",
    title: "Distributed Caching: Consistent Hashing Rings",
    creator: "Sanket Singh",
    domain: "Distributed Systems",
    difficulty: "Advanced",
    readTime: "14 min",
  },
  {
    id: "lru-implementation",
    type: "Tutorial",
    title: "Implementing an LRU Cache from scratch",
    creator: "Shivansh Kumar",
  ...
  },
  {
    id: "ai-roadmap",
    type: "Roadmap",
    title: "AI & Deep Learning Architect Roadmap",
    creator: "Quild_Engine Editorial",
    domain: "Artificial Intelligence",
    difficulty: "Intermediate",
    readTime: "12 steps",
  },
  {
    id: "compile-project",
    type: "Project",
    title: "Building a Sandboxed Compiler Execution Loop",
...
function ResearchPage() {
  const {
    ...
    paperId: activePaperIdParam,
    articleId: activeArticleIdParam,
    q: searchQueryParams,
  } = Route.useSearch();
  ...
  const setView = useCallback(
    (newView: ResearchView, extraParams: Partial<ResearchSearch> = {}) => {
      navigate({
        search: () => {
          ...
          if (extraParams.paperId !== undefined)
            nextSearch.paperId = extraParams.paperId;
          if (extraParams.articleId !== undefined)
            nextSearch.articleId = extraParams.articleId;
          return nextSearch;
        },
      });
    },
  ...
  const setActivePaperId = useCallback(
    (id: string) => {
  ...
  const setActiveArticleId = useCallback(
    (id: string) => {
  ...
  const [activeResearcherId, setActiveResearcherId] =
    useState<string>("elena-rostova");
  const [activeOrgId, setActiveOrgId] = useState<string>("antigravity-labs");
  const [activeCollectionId, setActiveCollectionId] =
    useState<string>("ai-learning");
  ...
  const activePaper = useMemo(
    () => mockPapers.find((p) => p.id === activePaperId) || mockPapers[0],
    [activePaperId],
  );
  const activeArticle = useMemo(
    () => mockArticles.find((a) => a.id === activeArticleId) || mockArticles[0],
    [activeArticleId],
  );
  ...
  const hasNext =
    mockPapers.findIndex((p) => p.id === activePaperId) < mockPapers.length - 1;
                ...
                <div className="py-8 px-6 max-w-5xl mx-auto space-y-12">
                  ...
                  <div className="text-center space-y-6 py-6 border-b border-[var(--sb-border)]/20">
                    ...
                    <p className="text-sm text-[var(--sb-ink-muted)] max-w-lg mx-auto">
                      Discover papers, technical articles, roadmaps, compiler
                      tutorials, and courses, integrated into a single
                      semantically connected graph.
                    </p>
                      ...
                      {mockCollections.map((col) => (
                        ...
                        >
                          <div className="space-y-1">
                            ...
                            <h4 className="font-extrabold text-sm text-[var(--sb-ink)] pt-1">
                              {col.name}
                            </h4>
                            <p className="text-xs text-[var(--sb-ink-muted)] line-clamp-2 leading-relaxed">
                              {col.description}
                            </p>
                          </div>
                        ...
                        {mockPapers.slice(0, 3).map((paper) => (
                          ...
                          >
                            <div className="space-y-2 max-w-[80%]">
                              ...
                              <h4 className="font-extrabold text-xs text-[var(--sb-ink)] leading-snug line-clamp-1">
                                {paper.title}
                              </h4>
                              <p className="text-[11px] text-[var(--sb-ink-muted)] line-clamp-2 leading-relaxed">
                                {paper.abstract}
                              </p>
                            ...
                            <div className="flex flex-col justify-between items-end text-[9px] text-[var(--sb-ink-dim)] shrink-0 font-mono">
                              <span className="bg-indigo-500/5 text-indigo-500 border border-indigo-500/10 px-2 py-0.5 rounded font-black">
                                {paper.duration}
                              </span>
                        ...
                        {mockResearchers.map((res) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <h4 className="font-bold text-xs text-[var(--sb-ink)] truncate">
                                {res.name}
                              </h4>
                              <p className="text-[10px] text-[var(--sb-ink-dim)] truncate">
                                {res.organization}
                              </p>
                              <p className="text-[9px] text-indigo-500 font-bold truncate pt-0.5">
                                {res.expertise.join(", ")}
                              </p>
                            </div>
                          </div>
                      ...
                      {mockOrganizations.map((org) => (
                        ...
                        >
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <h4 className="font-bold text-xs text-[var(--sb-ink)]">
                              {org.name}
                            </h4>
                          </div>
                          <p className="text-[11px] text-[var(--sb-ink-muted)] line-clamp-2 leading-relaxed">
                  ...
                  <div className="w-full md:w-60 shrink-0 space-y-5">
                    <div className="sticky top-20 p-4 border border-[var(--sb-border)]/55 bg-[color-mix(in_oklab,var(--card-bg)_65%,transparent)] backdrop-blur-sm rounded-xl space-y-4">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-[9px] font-bold uppercase tracking-wider text-[var(--sb-ink-dim)]">
                          Content Type
                        </label>
                        <select
                          value={filterType}
                    ...
                    <div className="space-y-3">
                        ...
                        <div
                          ...
                          onClick={() => {
                            if (item.type === "Paper") {
                              ...
                              executeAiAction(
                                `Tell me about the ${item.type}: "${item.title}" by ${item.creator}`,
                              );
                            }
                          }}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 space-y-1">
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] font-mono bg-[var(--sb-pill)] border border-[var(--sb-border)]/40 text-[var(--sb-ink-dim)] px-2 py-0.2 rounded font-bold">
                                {item.type}
                              </span>
                              <span className="text-[9px] font-mono text-[var(--sb-ink-dim)]">
                                {item.domain}
                              </span>
                            </div>
                            <h4 className="font-extrabold text-xs text-[var(--sb-ink)] line-clamp-1">
                              {item.title}
                            </h4>
                            <p className="text-[10px] text-[var(--sb-ink-dim)]">
                              By {item.creator} · Duration: {item.readTime}
                            </p>
                          </div>
                        </div>
                      ))}
                      ...
                      {filteredSearchItems.length === 0 && (
                        <div className="py-8 text-center text-xs text-[var(--sb-ink-dim)] italic">
                          No matching results found. Try clearing filters or
                          revising query.
                        </div>
                      )}
                    </div>
              ...
              {currentView === "feed" && (
                <div className="py-6 px-6 max-w-4xl mx-auto space-y-6">
                  <div className="border-b border-[var(--sb-border)]/30 pb-3">
                    <h2 className="text-xl font-extrabold tracking-tight text-[var(--sb-ink)]">
                      Infinite Research Feed
                    </h2>
                    <p className="text-[11px] text-[var(--sb-ink-muted)]">
                      Live-updating academic telemetry feed of trending and
                    ...
                    {mockPapers.map((paper) => (
                      ...
                      >
                        <div className="flex justify-between items-start gap-4">
                          <div className="space-y-1">
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] font-mono bg-indigo-500/10 border border-indigo-500/20 text-indigo-500 px-2 py-0.5 rounded uppercase font-black">
                                {paper.domain}
                              </span>
                              <span className="text-[9px] font-mono text-[var(--sb-ink-dim)]">
                                DOI: {paper.doi}
                          ...
                          <span className="font-mono text-[9px] bg-[var(--sb-pill)] px-2 py-0.5 rounded text-[var(--sb-ink-dim)] shrink-0">
                            {paper.duration}
                          </span>
                        </div>
                        ...
                        <div className="text-[10px] text-[var(--sb-ink-dim)] flex flex-wrap gap-x-4 gap-y-1 border-t border-[var(--sb-border)]/20 pt-3">
                          ...
                          <span>
                            Org:{" "}
                            <strong>{paper.organizations.join(", ")}</strong>
                          </span>
                          <span>·</span>
                          <span>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-[var(--sb-border)]/20 mt-1">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                executeAiAction(`Summarize ${paper.title}`)
                              }
                ...
                <div className="py-6 px-6 max-w-5xl mx-auto flex gap-6 relative animate-fadeIn select-text">
                  ...
                  <div className="flex-1 py-4 space-y-6 overflow-x-hidden relative">
                    ...
                    {hasPrev && (
                      <button
                        onClick={() => {
                          const currentIndex = mockPapers.findIndex(
                            (p) => p.id === activePaperId,
                          );
                          if (currentIndex > 0)
                            setActivePaperId(mockPapers[currentIndex - 1].id);
                        }}
                    ...
                    {hasNext && (
                      <button
                        onClick={() => {
                          ...
                          if (currentIndex < mockPapers.length - 1)
                            setActivePaperId(mockPapers[currentIndex + 1].id);
                        }}
                    ...
                    >
                      <header className="space-y-3 pb-6 border-b border-[var(--sb-border)]/40 pointer-events-none select-none">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[var(--sb-ink-dim)] pt-1">
                          <span>
                            Authors:{" "}
                            <strong>{activePaper.authors.join(", ")}</strong>
                          </span>
                          <span>·</span>
                          <span>
                            Orgs:{" "}
                            <strong>
                              {activePaper.organizations.join(", ")}
                            </strong>
                          </span>
                          <span>·</span>
                      ...
                      <div className="bg-[var(--sb-pill)]/50 border-l-4 border-indigo-500 rounded-r-xl p-5 shadow-sm">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-[var(--sb-ink)] mb-1">
                          Abstract Summary
                        </h4>
                        <p className="text-xs leading-relaxed text-[var(--sb-ink-muted)] italic">
                      ...
                      <div className="space-y-2 pt-2">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-indigo-500 flex items-center gap-1">
                          <Sparkles className="size-3.5 fill-current" /> Key AI
                          Findings
                        </h4>
                      ...
                      {activePaper.figure && (
                        <div className="border border-[var(--sb-border)]/40 rounded-xl overflow-hidden bg-[color-mix(in_oklab,var(--card-bg)_60%,transparent)] shadow-sm">
                          <div className="border-b border-[var(--sb-border)]/30 bg-[var(--sb-pill)] px-4 py-2 flex items-center justify-between">
                            <span className="text-[9px] font-mono uppercase tracking-wider text-[var(--sb-ink-dim)]">
                  ...
                  <div className="w-56 shrink-0 hidden lg:block space-y-4">
                    <div className="sticky top-20 p-4 border border-[var(--sb-border)]/55 bg-[color-mix(in_oklab,var(--card-bg)_65%,transparent)] rounded-xl space-y-4">
                      <div className="space-y-2 border-b border-[var(--sb-border)]/30 pb-3">
                        <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--sb-ink-dim)]">
                          Document Metadata
                        </span>
                      ...
                      <div className="flex flex-col gap-2 pt-1">
                        <button
                          onClick={() =>
                            executeAiAction(`Summarize this paper`)
                          }
                        ...
                        <button
                          onClick={() =>
                            executeAiAction(`Explain equations in this paper`)
                          }
                ...
                <div className="py-6 px-6 max-w-5xl mx-auto space-y-6">
                  ...
                  <div className="border-b border-[var(--sb-border)]/30 pb-3 flex justify-between items-end">
                    <div>
                      ...
                      <p className="text-[11px] text-[var(--sb-ink-muted)]">
                        Connect and follow leading scientists modeling
                        structural semantic systems.
                      </p>
                    </div>
                    <span className="text-[10px] text-[var(--sb-ink-dim)] font-mono">
                      Found {mockResearchers.length} researchers
                    </span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-extrabold text-xs text-[var(--sb-ink)]">
                                {res.name}
                              </h4>
                              <p className="text-[10px] text-[var(--sb-ink-dim)]">
                                {res.organization}
                          ...
                          <p className="text-[11px] text-[var(--sb-ink-muted)] leading-relaxed h-12 line-clamp-3">
                            {res.bio}
                          </p>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-center text-[10px] font-mono bg-[var(--sb-pill)] p-2.5 rounded-lg border border-[var(--sb-border)]/30">
                            <div>
                              <div className="font-black text-[var(--sb-ink)]">
                                {res.followers}
                              </div>
                              <div className="text-[8px] text-[var(--sb-ink-dim)] uppercase">
                                Followers
                              </div>
                            </div>
                            <div>
                          ...
                          <div className="flex justify-between items-center pt-2">
                            ...
                            <button
                              onClick={() => {
                                setActiveResearcherId(res.id);
                                executeAiAction(
                                  `Tell me about researcher ${res.name}`,
                                );
                              }}
                ...
                <div className="py-6 px-6 max-w-5xl mx-auto space-y-6">
                  <div className="border-b border-[var(--sb-border)]/30 pb-3 flex justify-between items-end">
                    <div>
                      ...
                      <p className="text-[11px] text-[var(--sb-ink-muted)]">
                        Browse technical publications and researchers associated
                        with major entities.
                      </p>
                    </div>
                    <span className="text-[10px] text-[var(--sb-ink-dim)] font-mono">
                      Found {mockOrganizations.length} institutions
                    </span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="space-y-3">
                            ...
                            <p className="text-[11px] text-[var(--sb-ink-muted)] leading-relaxed h-16 line-clamp-4">
                              {org.about}
                            </p>
                          </div>
                          ...
                          <div className="space-y-3 pt-3 border-t border-[var(--sb-border)]/20 mt-3">
                            <div className="flex flex-wrap gap-1 text-[8px] font-mono">
                              {org.domains.map((d) => (
                                <span
                                  key={d}
                                  className="bg-[var(--sb-pill)] border border-[var(--sb-border)]/30 px-2 py-0.5 rounded text-[var(--sb-ink-dim)]"
                                >
                                  {d}
                                </span>
                              ))}
                            ...
                            <div className="flex justify-between items-center text-[10px]">
                              <span className="font-mono text-[var(--sb-ink-dim)]">
                                {org.publicationsCount} papers cataloged
                              </span>
                              <button
                                onClick={() => {
                                  setActiveOrgId(org.id);
                                  executeAiAction(
                                    `List publications of ${org.name}`,
                                  );
                                }}
              ...
              {currentView === "collections" && (
                <div className="py-6 px-6 max-w-5xl mx-auto space-y-6">
                  <div className="border-b border-[var(--sb-border)]/30 pb-3 flex justify-between items-end">
                    <div>
                      <h2 className="text-xl font-extrabold tracking-tight text-[var(--sb-ink)]">
                        NotebookLM Curated Collections
                      </h2>
                      <p className="text-[11px] text-[var(--sb-ink-muted)]">
                        Upload resources or load curated folders to ask
                        questions about combined libraries.
                      </p>
                    </div>
                  </div>
                    ...
                    {mockCollections.map((col) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-start gap-4">
                            <div className="space-y-1">
                              <h3 className="font-extrabold text-sm text-[var(--sb-ink)]">
                                {col.name}
                              </h3>
                          ...
                          <div className="space-y-2 border-t border-[var(--sb-border)]/20 pt-3">
                            <span className="text-[9px] font-bold uppercase tracking-wider text-[var(--sb-ink-dim)] block">
                              Included Papers:
                            </span>
                            <div className="space-y-1.5">
                              {col.papers.map((pid) => {
                                ...
                                return (
                                  <div
                                    key={pid}
                                    className="flex justify-between items-center p-2 rounded bg-[var(--sb-pill)] border border-[var(--sb-border)]/30 text-[11px] text-[var(--sb-ink-muted)]"
                                  >
                          ...
                          <div className="pt-3 flex gap-2">
                            ...
                            >
                              <Sparkles className="size-3 fill-current" /> Ask
                              AI about Collection
                            </button>
                          </div>
                        </div>
                ...
                <div className="py-6 px-6 max-w-4xl mx-auto space-y-6">
                  ...
                  <div className="border-b border-[var(--sb-border)]/30 pb-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="space-y-1">
                      ...
                      <h2 className="text-2xl font-extrabold text-[var(--sb-ink)] pt-1">
                        {activeArticle.title}
                      </h2>
                      ...
                      {activeArticle.content.split("\n\n").map((para, k) => {
                        if (para.startsWith("## ") || para.startsWith("### ")) {
                          return (
                            <h3
                              key={k}
                              className="text-base font-bold display-title mt-4 border-b border-[var(--sb-border)]/40 pb-1 text-[var(--sb-ink)]"
                            >
                              {para.replace(/#+\s+/, "")}
                            </h3>
                          );
                        ...
                        return (
                          <p
                            key={k}
                            className="text-[var(--sb-ink-muted)] leading-relaxed"
                          >
                            {para}
                          </p>
                        );
                  ...
                  <div className="p-4 border border-indigo-500/10 rounded-xl bg-indigo-500/5 mt-6 space-y-3">
                    <span className="text-[10px] font-mono font-bold text-indigo-500 uppercase tracking-wider flex items-center gap-1">
                      <GraduationCap className="size-3.5" /> Interconnected
                      Learning Relations
                    </span>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      <div className="p-2 bg-[var(--card-bg)] border border-[var(--sb-border)]/40 rounded-lg text-[10px] font-sans">
                        <span className="text-[8px] font-mono text-[var(--sb-ink-dim)] block">
                          Related Course
                        </span>
                        <span className="font-extrabold text-[var(--sb-ink)] truncate block">
                          DSA Fundamentals
                        </span>
                      </div>
                      <div className="p-2 bg-[var(--card-bg)] border border-[var(--sb-border)]/40 rounded-lg text-[10px] font-sans">
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto py-4 space-y-4 scrollbar-none flex flex-col justify-between">
                ...
                <div className="space-y-4">
                  {aiChatLogs.map((chat, idx) => (
                    <div
                      key={idx}
                      className={`space-y-1 text-xs select-text ${chat.role === "user" ? "text-right" : "text-left"}`}
                    >
                      <span className="text-[8px] font-mono uppercase tracking-wider text-[var(--sb-ink-dim)] block">
                        {chat.role === "user" ? "You" : "Notebook AI"}
                      </span>
                      <div
                        className={`p-2.5 rounded-xl inline-block max-w-[90%] text-left whitespace-pre-wrap leading-relaxed ${
                          chat.role === "user"
                  ...
                  {aiChatLogs.length === 0 && (
                    <div className="p-4 border border-[var(--sb-border)]/45 rounded-xl bg-[var(--sb-pill)]/35 text-[11px] text-[var(--sb-ink-dim)] leading-relaxed italic text-center select-none">
                      Load a paper or curriculum collection, and ask me to
                      summarize it, explain complex equations, or generate
                ...
                <div className="space-y-1.5 pt-4 border-t border-[var(--sb-border)]/20 shrink-0">
                  <span className="text-[8px] font-black uppercase tracking-wider text-[var(--sb-ink-dim)] block px-1">
                    Quick Prompts
                  </span>
                  <button
                    type="button"
              ...
              <form
                ...
                className="pt-3 border-t border-[var(--sb-border)]/55 shrink-0 flex gap-2"
              >
                <input
                  type="text"
