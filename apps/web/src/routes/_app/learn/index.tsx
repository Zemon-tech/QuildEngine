...
import { BookOpen, ArrowRight, Flame, Award, Clock, Map } from "lucide-react";
    ...
    <div className="mx-auto max-w-6xl space-y-10">
      ...
      <div 
        ...
        style={{
          ...
          boxShadow: "0 1px 0 var(--inset-glint) inset, 0 10px 30px rgba(23, 58, 64, 0.05)"
        }}
      >
        <div className="space-y-2">
          ...
          <p className="text-sm max-w-md" style={{ color: "var(--sb-ink-muted)" }}>
            Your personalized curriculum is waiting. Continue building your low-level programming and system design mastery.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 md:gap-8 bg-[var(--sb-pill)] p-4 rounded-xl border border-[var(--sb-border)]">
          <div className="text-center">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider mt-1 text-[var(--sb-ink-dim)]">Daily Streak</p>
          </div>
          <div className="text-center border-x border-[var(--sb-border)] px-4">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider mt-1 text-[var(--sb-ink-dim)]">Lessons Done</p>
          </div>
          <div className="text-center">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider mt-1 text-[var(--sb-ink-dim)]">Spent Time</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold display-title flex items-center gap-2" style={{ color: "var(--sb-ink)" }}>
        ...
        {isLoading ? (
          <div className="grid gap-4 sm:grid-cols-2">
            {[1, 2].map((i) => (
              <div key={i} className="h-32 rounded-xl animate-pulse bg-[var(--sb-pill)] border border-[var(--sb-border)]" />
            ))}
          </div>
        ) : enrolledCourses.length === 0 ? (
          <div 
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {exploreCourses.map((course) => (
              ...
              >
                ...
                <p className="text-xs line-clamp-2" style={{ color: "var(--sb-ink-muted)" }}>
                  {course.description}
                </p>

                <div className="mt-auto pt-2 border-t border-[var(--sb-border)] flex items-center justify-between text-xs" style={{ color: "var(--sb-ink-dim)" }}>
      ...
      >
        <div className="flex gap-4 items-start">
          ...
          <div className="space-y-1">
            <h3 className="font-bold text-lg" style={{ color: "var(--sb-ink)" }}>Learning Roadmaps</h3>
            <p className="text-xs max-w-xl" style={{ color: "var(--sb-ink-muted)" }}>
              Navigate your computer science journey with interactive timelines guiding you from programming core fundamentals to expert backend and systems roles.
            </p>
          </div>
        </div>
        <Link
