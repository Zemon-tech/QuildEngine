        ...
        const defaultNotes: Note[] = [
          {
            ...
            content:
              "<p>Remember to review the context switching assembly routine next week. Context switching saves registers on the PCB (Process Control Block) and loads the new thread state.</p>",
            blocks: [
              { id: "b1", type: "h1", value: "Kernel Bootstrapping" },
              {
                ...
                type: "text",
                value:
                  "During boot, the BIOS initializes the system hardware and hands over control to the GRUB bootloader.",
              },
              {
                id: "b3",
                type: "code",
              ...
              },
              {
                id: "b4",
                type: "todo",
                value: "Implement GDT layout in C",
                completed: true,
              },
              {
                id: "b5",
                type: "todo",
                value: "Map memory pages",
                completed: false,
              },
            ],
          ...
          },
          {
            ...
            content:
              "<p>React Fiber splits rendering into render/reconciliation (which can be paused) and commit (which changes DOM, must be synchronous).</p>",
            blocks: [
              { id: "b6", type: "h2", value: "Concurrency features" },
              {
                ...
                type: "text",
                value:
                  "useTransition helps keep the UI responsive by deprioritizing heavy computations.",
              },
            ],
            editorType: "tiptap",
            updatedAt: new Date().toLocaleDateString(),
      ...
      blocks: [
        {
          id: Math.random().toString(),
          type: "text",
          value: "Start writing blocks here...",
        },
      ],
      editorType: "blocknote",
      updatedAt: new Date().toLocaleDateString(),
          ...
          <>
            ...
            <div className="p-4 border-b border-[var(--sb-border)] flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--sb-bg)]">
              ...
              <div className="flex bg-[var(--sb-pill)] p-1 rounded-lg border border-[var(--sb-border)] self-start text-[10px]">
                <button
                  onClick={() =>
                    handleUpdateActiveNote({ editorType: "blocknote" })
                  }
                ...
                <button
                  onClick={() =>
                    handleUpdateActiveNote({ editorType: "tiptap" })
                  }
                  className={`px-3 py-1 font-bold rounded transition-all cursor-pointer ${
        ...
        <button
          ...
          className={`p-1.5 rounded transition-all cursor-pointer ${
            editor.isActive("bold")
              ? "bg-[var(--sb-bg)] text-[var(--sb-accent)] border border-[var(--sb-border)]"
              : "text-[var(--sb-ink-dim)]"
          }`}
        >
          <strong>B</strong>
...
function BlockNoteEditorWrapper({ blocks, onChange }: BlockNoteProps) {
  ...
  const handleChangeBlockType = (
    id: string,
    type: Note["blocks"][0]["type"],
  ) => {
    const updated = blocks.map((b) =>
      b.id === id
        ? { ...b, type, completed: type === "todo" ? false : undefined }
        : b,
    );
    onChange(updated);
    ...
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="p-3 rounded-xl border border-teal-500/20 bg-teal-500/5 text-[10px] text-teal-600 dark:text-teal-400 flex items-center justify-between">
        <span className="flex items-center gap-1">
          <Sparkles size={12} /> Notion-Style BlockNote: Hover on any block
          block to change types or delete.
        </span>
      </div>
            ...
            <div className="absolute left-0 top-1 opacity-0 group-hover:opacity-100 flex items-center gap-1">
              ...
              <div className="relative">
                <button
                  onClick={() =>
                    setDropdownBlockId(
                      dropdownBlockId === block.id ? null : block.id,
                    )
                  }
                ...
                {dropdownBlockId === block.id && (
                  <div className="absolute left-0 top-6 z-20 w-32 border border-[var(--sb-border)] bg-[var(--sb-bg)] rounded-lg shadow-lg p-1 space-y-0.5 text-[9px] font-bold">
                    <button
                      onClick={() => handleChangeBlockType(block.id, "text")}
                      className="w-full text-left p-1.5 rounded hover:bg-[var(--sb-bg-hover)] flex items-center gap-1.5"
                    >
                      <Type size={10} /> Text
                    </button>
                    <button
                      onClick={() => handleChangeBlockType(block.id, "h1")}
                      className="w-full text-left p-1.5 rounded hover:bg-[var(--sb-bg-hover)] flex items-center gap-1.5"
                    >
                      <Heading1 size={10} /> Header 1
                    </button>
                    <button
                      onClick={() => handleChangeBlockType(block.id, "h2")}
                      className="w-full text-left p-1.5 rounded hover:bg-[var(--sb-bg-hover)] flex items-center gap-1.5"
                    >
                      <Heading2 size={10} /> Header 2
                    </button>
                    <button
                      onClick={() => handleChangeBlockType(block.id, "code")}
                      className="w-full text-left p-1.5 rounded hover:bg-[var(--sb-bg-hover)] flex items-center gap-1.5"
                    >
                      <Code size={10} /> Code Block
                    </button>
                    <button
                      onClick={() => handleChangeBlockType(block.id, "todo")}
                      className="w-full text-left p-1.5 rounded hover:bg-[var(--sb-bg-hover)] flex items-center gap-1.5"
                    >
            ...
            <div className="flex-1">
              {block.type === "text" && (
                <textarea
                  value={block.value}
                  onChange={(e) =>
                    handleUpdateBlockValue(block.id, e.target.value)
                  }
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className={`w-full bg-transparent focus:outline-none text-sm text-[var(--sb-ink)] ${
                      block.completed
                        ? "line-through text-[var(--sb-ink-dim)] opacity-60"
                        : ""
                    }`}
                  />
                </div>
