...
import { Bookmark, Clock, Play, Trash2, Star, ArrowRight } from "lucide-react";
import { useBookmarks, useToggleCourseBookmark, useToggleLessonBookmark, useCourses } from "#/hooks/use-courses";
...
export const Route = createFileRoute("/_app/courses/bookmarks")({
  validateSearch: (search: Record<string, unknown>): BookmarksSearch => {
    return {
      tab: (search.tab as "courses" | "lessons" | "roadmaps" | undefined) || "courses",
    };
  },
...
function BookmarksPage() {
  ...
  const bookmarkedCourses = courses?.filter(c => bookmarks?.courses.includes(c.id)) || [];
  const bookmarkedLessons = bookmarks?.lessons || [];

  const bookmarkedNodes = (progress?.bookmarkedNodes ?? []).map((nodeId) => {
  ...
  const favoritedRoadmaps = (progress?.favorites ?? []).map((roadmapId) => {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {/* Main Panel Content */}
      {isLoading ? (
        <div className="p-12 text-center animate-pulse">Loading bookmarks...</div>
      ) : activeTab === "courses" ? (
        /* Courses Tab */
        ...
        bookmarkedLessons.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-[var(--sb-border)] rounded-xl bg-[var(--sb-bg-hover)]/30 space-y-4">
            ...
            <p className="text-xs" style={{ color: "var(--sb-ink-dim)" }}>
              Enroll in a course and click the bookmark flag next to any lesson to save it here.
            </p>
          </div>
        ) : (
        ...
        <div className="space-y-10">
          ...
          <div className="space-y-4">
            <h2 className="text-sm font-bold flex items-center gap-2" style={{ color: "var(--sb-ink)" }}>
            ...
            {favoritedRoadmaps.length === 0 ? (
              <div className="text-center py-10 border border-dashed border-[var(--sb-border)] rounded-xl bg-[var(--sb-bg-hover)]/30 space-y-2">
                <p className="text-xs font-medium" style={{ color: "var(--sb-ink-muted)" }}>
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="space-y-1">
                        ...
                        <h3 className="font-bold text-base leading-tight mt-1 group-hover:text-[var(--sb-accent)] transition-colors duration-200" style={{ color: "var(--sb-ink)" }}>
                          {pathway.title}
                        </h3>
                      </div>
                      <button
                    ...
                    <p className="text-xs line-clamp-2 leading-relaxed" style={{ color: "var(--sb-ink-muted)" }}>
                      {pathway.description}
                    </p>

                    <div className="mt-auto space-y-3">
                      <div className="flex items-center justify-between text-[11px] border-t border-[var(--sb-border)] pt-3" style={{ color: "var(--sb-ink-dim)" }}>
                      ...
                      {pathway.progress > 0 && (
                        <div className="space-y-1">
                          <div className="flex justify-between items-center text-[10px] font-medium">
                            <span style={{ color: "var(--sb-ink-dim)" }}>Progress</span>
                            <span className="text-[var(--sb-accent)]">{pathway.progress}%</span>
                          </div>
                          <div className="h-1 w-full rounded-full overflow-hidden bg-[var(--sb-pill)]">
                            <div className="h-full bg-[var(--sb-accent)]" style={{ width: `${pathway.progress}%` }} />
                          </div>
                        </div>
                      )}

                      <button
                        onClick={() => navigate({ to: "/learn/roadmaps", search: { id: pathway.roadmapId } })}
                        className="w-full py-2 text-xs font-bold rounded-lg cursor-pointer bg-[var(--sb-bg-hover)] text-[var(--sb-ink)] hover:bg-[var(--sb-bg-active)] border border-[var(--sb-border)] transition-colors flex items-center justify-center gap-1"
                      >
                ...
                {bookmarkedNodes.map((topic) => (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-xs line-clamp-1" style={{ color: "var(--sb-ink-muted)" }}>
