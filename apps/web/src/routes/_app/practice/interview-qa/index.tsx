...
import { Brain, Code2, HelpCircle, Trophy, Users } from "lucide-react";
import { useInterviewQA } from "#/hooks/use-practice";

export const Route = createFileRoute("/_app/practice/interview-qa/")({
  component: InterviewQALandingPage,
});
...
const colorGradients: Record<string, string> = {
  technical: "from-blue-500/10 to-cyan-500/10 border-blue-500/20 text-blue-500",
  behavioral:
...
function InterviewQALandingPage() {
  const { data: categories = [], isLoading } = useInterviewQA();
  ...
  return (
    <div className="w-full flex-1 min-w-0 px-4 md:px-6 lg:px-8 py-2 space-y-8">
      ...
      >
        <div>
          ...
          >
            Review key technical interview topics, practice behavioral stories,
            and practice AI-generated scenarios.
          </p>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          <HelpCircle size={18} className="text-[var(--sb-accent)]" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {categories.map((cat) => {
            const IconComponent = iconMap[cat.id] || HelpCircle;
            const progress =
            ...
            return (
              ...
              >
                <motion.div
                  ...
                  className="flex h-[220px] flex-col justify-between rounded-2xl border p-5 shadow-xs hover:shadow-md cursor-pointer select-none transition-all"
                  style={{
                    background: "var(--card-bg)",
                    borderColor: "var(--card-border)",
                  }}
