...
import { useInterviewQACategory } from "#/hooks/use-practice";

export const Route = createFileRoute("/_app/practice/interview-qa/$type")({
  component: InterviewQATypePage,
});
...
function InterviewQATypePage() {
  const { type } = Route.useParams();
  const { data: category, isLoading, error } = useInterviewQACategory(type);
    ...
    return (
      <div className="flex h-64 flex-col items-center justify-center gap-3">
        ...
        <p className="text-sm font-semibold" style={{ color: "var(--sb-ink)" }}>
          Q&A Category not found or failed to load.
        </p>
        <Link
      ...
      >
        ...
        >
          ...
          <div className="text-[10px]" style={{ color: "var(--sb-ink-dim)" }}>
            {category.completedQuestions} / {category.totalQuestions} questions
            cleared
          </div>
        </div>
      </div>
