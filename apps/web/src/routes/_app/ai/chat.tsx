...
import { Sparkles, MessageSquare, BookOpen, Code2 } from "lucide-react";

export const Route = createFileRoute("/_app/ai/chat")({ component: AIChatPage });

const modes = [
  { icon: MessageSquare, label: "General", desc: "Ask anything" },
  { icon: BookOpen, label: "Course Assistant", desc: "Help with your courses" },
  { icon: Code2, label: "Coding Assistant", desc: "Debug, explain, and review code" },
];
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--sb-ink)", fontFamily: "'Fraunces', Georgia, serif" }}>AI Chat</h1>
          <p className="text-sm" style={{ color: "var(--sb-ink-muted)" }}>Choose a mode and start a conversation.</p>
        </div>
      </div>
      <div className="grid gap-3 sm:grid-cols-3">
        {modes.map(({ icon: Icon, label, desc }) => (
          ...
          >
            <div className="flex size-9 items-center justify-center rounded-lg" style={{ background: "var(--sb-pill)" }}>
