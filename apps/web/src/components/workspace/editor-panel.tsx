...
const PROBLEM_SIGNATURES: Record<
  string,
  {
  ...
  if (spec) {
    if (lang === "typescript" || lang === "javascript") {
      const args = spec.params
        ...
        .join(", ");
      const ret =
        lang === "typescript" && spec.returnType.ts
          ? `: ${spec.returnType.ts}`
          : "";
      const defaultVal =
        spec.returnType.ts === "boolean"
          ? "true"
          : spec.returnType.ts === "number[]"
            ? "[0, 1]"
            : spec.returnType.ts === "number"
              ? "0"
              : "[]";
...
export function EditorPanel({ problemId }: EditorPanelProps) {
  ...
  const [lastSaved, setLastSaved] = useState<string>(() => {
    if (typeof window !== "undefined") {
      return (
        localStorage.getItem(`quild_last_saved_${problemId}_${language}`) || ""
      );
    }
    return "";
  });
  ...
  const handleEditorDidMount: OnMount = (editor, monaco) => {
    ...
    const savedPos = localStorage.getItem(
      `monaco_cursor_${problemId}_${language}`,
    );
    if (savedPos) {
      try {
    ...
    const savedScroll = localStorage.getItem(
      `monaco_scroll_${problemId}_${language}`,
    );
    if (savedScroll) {
      try {
  ...
  useEffect(() => {
    if (saveStatus === "saving") {
      const timer = setTimeout(() => {
        ...
        localStorage.setItem(
          `quild_last_saved_${problemId}_${language}`,
          timeStr,
        );
      }, 1000);
  ...
  const code =
    codeDrafts[draftKey] || getDefaultCodeTemplate(problem, language);
        ...
        <div className="flex h-10 shrink-0 items-center border-b border-[#1A1A1A] bg-[#111111] px-3">
          ...
          <div className="flex items-center gap-3 justify-center">
            ...
            <Button
              ...
              onClick={() =>
                document.getElementById("quild-run-button")?.click()
              }
            ...
            <Button
              ...
              disabled={isRunning || isSubmitting}
              onClick={() =>
                document.getElementById("quild-submit-button")?.click()
              }
